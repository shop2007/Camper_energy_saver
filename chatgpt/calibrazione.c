...
void Calibrazione() {
  ...
  Calibrazione_servizi = volt_trimmer_srv / (analogRead(VBAT_SERVIZI_PIN) * VREF / 1023.0 * (R1 + R2) / R2);
  Calibrazione_motore  = volt_trimmer_mot / (analogRead(VBAT_MOTORE_PIN) * VREF / 1023.0 * (R1 + R2) / R2);
